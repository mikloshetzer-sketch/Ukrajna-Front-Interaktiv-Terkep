...
def save_png(image_bytes: bytes, lat: float, lon: float) -> dict:
    LATEST_IMAGE_PATH.write_bytes(image_bytes)

    timestamp = utc_now().strftime("%Y%m%dT%H%M%SZ")
    history_name = f"sentinel2_{timestamp}_{safe_coord(lat)}_{safe_coord(lon)}.png"
    history_path = SENTINEL2_HISTORY_DIR / history_name
    history_path.write_bytes(image_bytes)

    return {
        "latest": "data/satellite/sentinel2/latest.png",
        "history": f"data/satellite/sentinel2/history/{history_name}",
...
def write_metadata(
    ...
    metadata = {
        **existing,
        ...
        "next_step": "Add Sentinel-2 image overlay to the Leaflet map.",
    }
...
def main() -> None:
    ...
    image_paths = save_png(image_bytes=image_bytes, lat=lat, lon=lon)
